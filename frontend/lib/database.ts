...
export class DatabaseService {
  ...
  async getPlayers(gameId: string): Promise<Player[]> {
    try {
      const result = await this.pool.query(
        'SELECT * FROM players WHERE game_id = $1 ORDER BY signed_up_at',
        [parseInt(gameId)]
      )
      return result.rows
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        for (const assignment of assignments) {
          await client.query(
            'UPDATE players SET role = $1, is_wolf = $2 WHERE id = $3 AND game_id = $4',
            [assignment.role, assignment.isWolf, assignment.playerId, parseInt(gameId)]
          )
        }
